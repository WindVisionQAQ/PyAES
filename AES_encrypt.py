...
# Key used in AES encryption
key = [0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0A,0x0B,0x0C,0x0D,0x0E,0x0F]
# plain_text is the input code clients are going to encrypt.
...
def key_expansion(key):
	...
	return round_key
# key state rotate in key expansion
def key_rotate(temp):
	t = temp[0]
	for i in range(1,4):
		temp[i-1] = temp[i]
	temp[3] = t
	return temp
# key state substitute in key expansion
def key_sub(temp):
	for i in range(4):
		temp[i] = s_box[temp[i]]
	return temp
...
# state substitute using s_box in encryption
...
state = aes_encrypt(plain_text,round_key)
for i in range(16):
